...
object ChannelStats {
  ...
  def main(args: Array[String]) {
    ...
    val raw_channel_path = sqlContext.read.parquet(commonUtils.raw_path_channel_data)
    raw_channel_path.registerTempTable("CHANNEL")
    raw_channel_path.printSchema()
    val channel_df_duration = sqlContext.sql("SELECT duration,count(*) FROM CHANNEL GROUP BY duration")
    channel_df_duration.show()

    val channel_df_users = sqlContext.sql("SELECT SK_DEVICE_ID,count(distinct program_id) FROM CHANNEL where program_id != 0 GROUP BY SK_DEVICE_ID ")
    channel_df_users.show()

    val channel_df_date = sqlContext.sql("SELECT EVENT_DATE,AVG(duration) FROM")
